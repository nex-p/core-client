...
interface QueryVerbalOptions {
  fieldLabels?: Record<string, string>; // Map field names to friendly labels
  valueLabels?: Record<string, Record<string, string>>; // Map field values to labels
  includeStructural?: boolean; // Include select, limit, offset
  conjunction?: 'and' | 'or'; // How to join multiple filters
}
...
function formatFieldName(field: string, labels?: Record<string, string>): string {
...
function parseFilterToVerbal(
  field: string,
  value: string,
  options: QueryVerbalOptions
): string | null {
